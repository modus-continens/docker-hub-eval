...
function update_node_version() {
  ...
  (
    ...
    echo "node_version(\"$nodeVersion\", \"$variant\", \"$arch\", \"$yarnVersion\")."
    ...
    if is_alpine "${variant}"; then
      ...
      echo "alpine_binary(\"$nodeVersion\", \"$arch\", \"$checksum\")."
    fi
  )
}
...
for version in "${versions[@]}"; do
  ...
  if [ -f "${version}/Dockerfile" ]; then
    if [ "${update_version}" -eq 0 ]; then
      update_node_version "${baseuri}" "${versionnum}" || exit $?
    fi
  fi
  ...
  for variant in "${variants[@]}"; do
    ...
    if [ "${update_version}" -eq 0 ] && [ "${update_variant}" -eq 0 ]; then
      update_node_version "${baseuri}" "${versionnum}" "${variant}" || exit $?
    fi
  done
done
